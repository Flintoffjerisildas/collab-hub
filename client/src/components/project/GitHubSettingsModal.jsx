...
const GitHubSettingsModal = ({ project, onClose, onUpdate }) => {
    ...
    const [activeTab, setActiveTab] = useState('settings'); // 'settings' or 'commits'
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-2xl rounded-lg bg-card p-6 shadow-lg border max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex gap-2">
                        <button
                            className={`px-3 py-1 rounded-md text-sm font-medium transition-colors ${activeTab === 'settings' ? 'bg-primary/10 text-primary' : 'hover:bg-muted'}`}
                        ...
                        {project.githubRepoName && (
                            <button
                                className={`px-3 py-1 rounded-md text-sm font-medium transition-colors ${activeTab === 'commits' ? 'bg-primary/10 text-primary' : 'hover:bg-muted'}`}
                ...
                {activeTab === 'commits' && (
                    <GitHubCommitList projectId={project._id} />
                )}
